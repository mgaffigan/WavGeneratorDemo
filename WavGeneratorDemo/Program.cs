    ...
    namespace WavGeneratorDemo
    {
        class Program
        {
            ...
            static void Main(string[] args)
            {
                ...
                var samples = new int[lengthInSeconds * sampleRate * channels];
                ...
                using (var wavFile = File.OpenWrite("out.wav"))
                {
                    ...
                    wavFile.WriteLittleEndianUInt16(3 * channels);    // Block Align (stride)
                    ...
                    {
                        wavFile.WriteLittleEndianInt24(samples[i]);
                    }
                }
            }

            private static void FillWithSineWave(int[] samples, int channels, double channelSamplesPerSecond, int channelNo, double freq)
            {
                for (int i = channelNo; i < samples.Length; i += channels)
                {
                    var t = (i - channelNo) / channelSamplesPerSecond;
                    var value = Math.Sin(t * (freq * Math.PI * 2));

                    samples[i] = DoubleToInt24(value);
                }
            }
